...
- Rewrite decision (post-migration): migrated salidas carry no stock link (stock_id NULL — source '09.SalidaStock' has no such column), so the edit pencil now shows on ANY non-returned row and `editarSalida` branches on stock_id: WITH a stock_id it reconciles the credited stock row + writes the audit movimiento (PA parity, `stock_editar_salida`); with stock_id NULL it corrects the record only (updates Cantidad_SS, no reconcile, no audit) — reconciling a months-old row would move today's inventory and the source can't say which shelf to credit (traslados also lack the destination link). source: `services/mock/adapter.ts` (editarSalida stock_id==null branch), `supabase/migrations/0011_salida_editar_record_only.sql`, `components/stock/SalidasStockView.tsx:206` (puedeEditar = !fecha_reingreso)
